...
using DesktopClient.Model.Gaming;
...
namespace DesktopClient
{
    public partial class MainWindow : Window
    {
        ...
        public void MakeMove(string position, Move.MoveEnum move)
        {
            foreach (Button button in BattleGrid.Children)
            {
                if (Regex.IsMatch(button.Name, string.Format("{0}$",position)))
                {
                    button.Content = move == Move.MoveEnum.Your ? YourFigure : OpponentFigre;
                }
            }
        }

        internal void CleanButtonsContent()
        {
            foreach (Button button in BattleGrid.Children)
            {
                button.Content = null;
